...
def conv_cond_concat(x, y, scale = 1 ):
  """Concatenate conditioning vector on feature map axis."""
  x_shapes = x.get_shape()
  y_shapes = y.get_shape()
  return concat([
    x, y*scale*tf.ones([x_shapes[0], x_shapes[1], x_shapes[2], y_shapes[3]])], 3)
...
def permute_dim( matrix, z_dim ):
    for i in range( z_dim ):
        np.random.shuffle( matrix[:, i] )
    return  matrix 

def calc_kl( mu, logvar ):
    return tf.reduce_sum( 0.5 * ( -1. - logvar + tf.square(mu) + tf.exp( logvar ) ) )

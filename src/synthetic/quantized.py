...
def get_sample_index_from_tick( cur_tick, tick_per_beat, tempo, sample_rate, padd_start = 72 ):
	beat = cur_tick / tick_per_beat
	ms = beat * tempo 
	return math.ceil( ms / 1000000 * sample_rate ) + padd_start
...
def create_midi_file( file_name, length, instrument = 1,
	...
	num_sample_float = length / 1000000 * sample_rate
	num_beat = length / tempo
	num_tick = math.floor( mid.ticks_per_beat * num_beat )
	num_tick_float = mid.ticks_per_beat * num_beat

	num_sample_per_tick_float = num_sample_float / num_tick_float
	...
	one_second_tick = 960 #int( sample_rate / num_sample_per_tick_float )
	print("one sec tick", one_second_tick)
	print("")
	start_tick = one_second_tick
	sound_length = one_second_tick
	lower, upper = _range
	# if single note
	if simul == 1:
	...
	# label[ get_sample_index_from_tick( sample_rate / num_sample_per_tick_float, mid.ticks_per_beat, tempo, sample_rate ) ] = 1.
	label[ 44100 -1 ] = 1
	if include_offset:
...
skip_list = []
os.mkdir( "quantized" ) if not os.path.isdir( "quantized" ) else print()
for instrument_index, name in name_dict.items():
	try:
		instrument = name
		_range = range_dict[ instrument_index ]
		base_dir = "quantized/" + instrument
